...
# 前端可选的完整模型列表（mode 直接存模型 ID）。claude-deepseek-v4.1-flash[1m]
# 放首位——列表首位即前端下拉的默认项（前端另有静态表，见 web/index.html）。
CLAUDE_MODELS = [
...
# 新会话默认模型：claude-deepseek-v4.1-flash[1m]（glm-5.3[1m] 保留为可选项）。
# （旧档位 fast/strong/super 仍兼容，见 session_hub 的 _LEGACY_MAP。）
CLAUDE_DEFAULT_MODE = os.environ.get("CLAUDE_DEFAULT_MODE", "claude-deepseek-v4.1-flash[1m]")
...
# 当前仅对零流量的 claude 路径生效（codex 路径刻意不传 effort，见 codex_oneshot.py 的
# prewarm bug 记录），不要把它当成能调 codex 侧验证/摘要深度的旋钮。
